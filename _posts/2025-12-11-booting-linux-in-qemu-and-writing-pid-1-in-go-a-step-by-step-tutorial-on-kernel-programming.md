...
הפקודה `make -j $(nproc)` משתמשת בכל ליבות המעבד שלך כדי לזרז את תהליך הקומפילציה.
...
כעת, נכתוב תהליך PID 1 בשפת Go. ניצור קובץ חדש בשם `init.go`:
...
בקוד הזה, אנחנו מגדירים תהליך PID 1 שמדפיס הודעת ברכה, מפעיל מעטפת קונסולה (`/bin/sh`), וממתין לסיום התהליך הזה לפני שהוא מסיים את עבודתו.
...
כדי לשלב את תהליך PID 1 בליבה, נצטרך לשנות את קובץ `init/main.c` בתיקיית הליבה. נוסיף את השורות הבאות בתחילת הפונקציה `start_kernel`:
...
- **שימוש ב-Optimization Flags**: כאשר אתה מקמפל את הליבה, השתמש בדגלי אופטימיזציה כמו `-O2` כדי לשפר את הביצועים.
